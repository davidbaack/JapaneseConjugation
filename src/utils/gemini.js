...
export const AI_FEEDBACK_LEVELS = [
  {
    ...
    label: 'Beginner',
    prompt:
      'Use plain English, short sentences, kana plus romaji for every key Japanese form, and no more than one new grammar term at a time.',
  },
  {
    id: 'expert',
    label: 'Expert JP',
    prompt:
      'Give compact, Japanese-heavy feedback. Use Japanese grammar terms when useful, include romaji only after the most important forms, and challenge the learner with one harder contrast.',
  },
];
...
export const AI_GUIDE_TONES = [
  {
    id: 'sensei',
    label: 'Calm',
    prompt: 'Sound like a patient teacher: warm, precise, and encouraging without being wordy.',
  },
  {
    id: 'coach',
    label: 'Coach',
    prompt: 'Sound like an energetic study coach: upbeat, direct, and momentum-building.',
  },
  {
    id: 'direct',
    label: 'Direct',
    prompt:
      'Sound like a concise examiner: minimal praise, clear correction, and one exact next action.',
  },
];
...
export function normalizeScannerGroup(v) {
  if (!v) return null;
  if (
    v.includes('i-adjective') ||
    v.includes('i adjective') ||
    v.includes('i-adj') ||
    v.includes('い-adj') ||
    v.includes('い adjective') ||
    v.includes('い形容詞')
  )
    return 'i-adjective';
  if (
    v.includes('na-adjective') ||
    v.includes('na adjective') ||
    v.includes('na-adj') ||
    v.includes('な-adj') ||
    v.includes('な adjective') ||
    v.includes('な形容詞')
  )
    return 'na-adjective';
  ...
  return rows
    .map((row) => {
      ...
      return {
        ...
        meaning,
        group,
        evidence: String(row.evidence || row.surface || '').trim(),
        ...(jlpt ? { jlpt } : {}),
      };
    })
    .filter(Boolean);
