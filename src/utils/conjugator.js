...
export function filterWordsForPrefs(words, prefs = DEFAULT_PREFS, wordLists = []) {
  ...
  const types =
    prefs.wordTypes && prefs.wordTypes.length
      ? prefs.wordTypes
      : WORD_TYPE_OPTIONS.map((o) => o.id);
  const groups =
    prefs.wordGroups && prefs.wordGroups.length
      ? prefs.wordGroups
      : WORD_GROUP_OPTIONS.map((o) => o.id);
  const lessonFilter =
    Array.isArray(prefs.genkiLessons) &&
    prefs.genkiLessons.length &&
    prefs.genkiLessons.length < GENKI_LESSONS.length
      ? new Set(prefs.genkiLessons.map(Number))
      : null;
  const minnaFilter =
    Array.isArray(prefs.minnaLessons) &&
    prefs.minnaLessons.length &&
    prefs.minnaLessons.length < MINNA_LESSONS.length
      ? new Set(prefs.minnaLessons.map(Number))
      : null;
  const selectedLists = prefs.wordListIds && prefs.wordListIds.length ? prefs.wordListIds : [];
  const allowedKeys = selectedLists.length
    ? new Set(
        wordLists.filter((l) => selectedLists.includes(l.id)).flatMap((l) => l.wordKeys || []),
      )
    : null;
...
export function typePreviewValues(typeId) {
  return typePreviewItems(typeId)
    .map((item) => ({ item, answer: conjugateItem(item, typeId) }))
    .filter((x) => x.answer);
}
...
export const ONBIN_PATTERN_META = {
  ...
  iku: {
    label: '行く exception',
    cue: '行く is the famous exception: いって / いった.',
    group: 'っ',
  },
...
export function desiderativeStem(verb) {
  ...
  if (group === 'godan') {
    ...
    return reading.slice(0, -1) + I_ROW[last];
  }
...
export function shortCausativePassive(verb) {
  ...
  if (group === 'kuru')
    return (reading.endsWith('くる') ? reading.slice(0, -2) : '') + 'こさされる';
  return '';
}
...
export function conjugateShortCausative(verb, targetType) {
  ...
  return form
    ? conjugate({ reading: form, dict: form, meaning: verb.meaning, group: 'godan' }, targetType)
    : '';
}
...
export function regularHonorificForm(verb) {
  ...
  if (verb.group === 'kuru')
    return (verb.reading.endsWith('くる') ? verb.reading.slice(0, -2) : '') + 'いらっしゃる';
...
export function regularHumbleForm(verb) {
  ...
  if (verb.group === 'kuru')
    return (verb.reading.endsWith('くる') ? verb.reading.slice(0, -2) : '') + 'まいる';
  const stem = conjugate(verb, 'masu-stem');
...
export function politeRuForm(form) {
  return form && form.endsWith('る') ? form.slice(0, -1) + 'ます' : '';
}
export function politePastRuForm(form) {
  return form && form.endsWith('る') ? form.slice(0, -1) + 'ました' : '';
}
export function pastRuForm(form) {
  return form && form.endsWith('る') ? form.slice(0, -1) + 'た' : '';
}
export function pastNegativeRuForm(form) {
  return form && form.endsWith('る') ? form.slice(0, -1) + 'なかった' : '';
}
export function conditionalRuForm(form) {
  return form && form.endsWith('る') ? form.slice(0, -1) + 'れば' : '';
}
export function negativeRuForm(form) {
  return form && form.endsWith('る') ? form.slice(0, -1) + 'ない' : '';
}
export function politeNegativeRuForm(form) {
  return form && form.endsWith('る') ? form.slice(0, -1) + 'ません' : '';
}
export function politePastNegativeRuForm(form) {
  return form && form.endsWith('る') ? form.slice(0, -1) + 'ませんでした' : '';
}
export function negativeBaForm(form) {
  return form && form.endsWith('ない') ? form.slice(0, -2) + 'なければ' : '';
}
export function negativeTeConnectiveForm(form) {
  return form && form.endsWith('ない') ? form.slice(0, -2) + 'なくて' : '';
}
...
export function negativeZuniForm(verb) {
  const form = negativeZuForm(verb);
  return form ? form + 'に' : '';
}
...
export function conjugate(verb, type) {
  ...
  const shortCausativeBase = {
    'short-causative': 'plain-present',
    'short-causative-polite': 'polite-present',
    'short-causative-negative': 'plain-negative',
    'short-causative-polite-negative': 'polite-negative',
    'short-causative-past': 'plain-past',
    'short-causative-polite-past': 'polite-past',
    'short-causative-past-negative': 'plain-past-negative',
    'short-causative-polite-past-negative': 'polite-past-negative',
    'short-causative-conditional-ba': 'conditional-ba',
    'short-causative-negative-conditional-ba': 'negative-conditional-ba',
  }[type];
  if (shortCausativeBase) return conjugateShortCausative(verb, shortCausativeBase);
  const politeBase = {
    'potential-polite': 'potential',
    'passive-polite': 'passive',
    'causative-polite': 'causative',
    'causative-passive-polite': 'causative-passive',
    'short-causative-passive-polite': 'short-causative-passive',
  }[type];
  if (politeBase) return politeRuForm(conjugate(verb, politeBase));
  const politePastBase = {
    'potential-polite-past': 'potential',
    'passive-polite-past': 'passive',
    'causative-polite-past': 'causative',
    'causative-passive-polite-past': 'causative-passive',
    'short-causative-passive-polite-past': 'short-causative-passive',
  }[type];
  if (politePastBase) return politePastRuForm(conjugate(verb, politePastBase));
  const pastBase = {
    'potential-past': 'potential',
    'passive-past': 'passive',
    'causative-past': 'causative',
    'causative-passive-past': 'causative-passive',
    'short-causative-passive-past': 'short-causative-passive',
  }[type];
  if (pastBase) return pastRuForm(conjugate(verb, pastBase));
  const pastNegBase = {
    'potential-past-negative': 'potential',
    'passive-past-negative': 'passive',
    'causative-past-negative': 'causative',
    'causative-passive-past-negative': 'causative-passive',
    'short-causative-passive-past-negative': 'short-causative-passive',
  }[type];
  if (pastNegBase) return pastNegativeRuForm(conjugate(verb, pastNegBase));
  const conditionalBase = {
    'potential-conditional-ba': 'potential',
    'passive-conditional-ba': 'passive',
    'causative-conditional-ba': 'causative',
    'causative-passive-conditional-ba': 'causative-passive',
    'short-causative-passive-conditional-ba': 'short-causative-passive',
  }[type];
  if (conditionalBase) return conditionalRuForm(conjugate(verb, conditionalBase));
  const negativeBase = {
    'causative-passive-negative': 'causative-passive',
    'short-causative-passive-negative': 'short-causative-passive',
  }[type];
  if (negativeBase) return negativeRuForm(conjugate(verb, negativeBase));
  const negativeBaBase = {
    'negative-conditional-ba': 'plain-negative',
    'potential-negative-conditional-ba': 'potential-negative',
    'passive-negative-conditional-ba': 'passive-negative',
    'causative-negative-conditional-ba': 'causative-negative',
    'causative-passive-negative-conditional-ba': 'causative-passive-negative',
    'short-causative-passive-negative-conditional-ba': 'short-causative-passive-negative',
  }[type];
  if (negativeBaBase) return negativeBaForm(conjugate(verb, negativeBaBase));
  if (type === 'negative-te-connective')
    return negativeTeConnectiveForm(conjugate(verb, 'plain-negative'));
  if (type === 'negative-zu') return negativeZuForm(verb);
  if (type === 'negative-zuni') return negativeZuniForm(verb);
  const progressiveEnding = {
    progressive: 'いる',
    'progressive-polite': 'います',
    'progressive-negative': 'いない',
    'progressive-polite-negative': 'いません',
    'progressive-past': 'いた',
    'progressive-polite-past': 'いました',
    'progressive-past-negative': 'いなかった',
    'progressive-polite-past-negative': 'いませんでした',
  }[type];
  ...
  const politeNegBase = {
    'potential-polite-negative': 'potential',
    'passive-polite-negative': 'passive',
    'causative-polite-negative': 'causative',
    'causative-passive-polite-negative': 'causative-passive',
    'short-causative-passive-polite-negative': 'short-causative-passive',
  }[type];
  if (politeNegBase) return politeNegativeRuForm(conjugate(verb, politeNegBase));
  const politePastNegBase = {
    'potential-polite-past-negative': 'potential',
    'passive-polite-past-negative': 'passive',
    'causative-polite-past-negative': 'causative',
    'causative-passive-polite-past-negative': 'causative-passive',
    'short-causative-passive-polite-past-negative': 'short-causative-passive',
  }[type];
  ...
  if (type === 'desiderative-polite-past-negative')
    return desiderativeStem(verb) + 'たくなかったです';
  if (group === 'ichidan') {
    const stem = reading.slice(0, -1);
    const te = stem + 'て',
      neg = stem + 'ない',
    ...
    const M = {
      ...
      'polite-te': stem + 'まして',
      'polite-conditional-tara': stem + 'ましたら',
      'te-form': te,
      potential: stem + 'られる',
      'potential-negative': stem + 'られない',
      volitional: stem + 'よう',
      'conditional-tara': past + 'ら',
      'negative-conditional-tara': stem + 'なかったら',
      'conditional-ba': stem + 'れば',
      'conditional-nara': reading + 'なら',
      conjectural: reading + 'だろう',
      imperative: stem + 'ろ',
      passive: stem + 'られる',
      'passive-negative': stem + 'られない',
      causative: stem + 'させる',
      'causative-negative': stem + 'させない',
      'causative-passive': stem + 'させられる',
      desiderative: stem + 'たい',
      progressive: te + 'いる',
      'negative-te': neg + 'で',
      prohibition: reading + 'な',
      'request-kudasai': te + 'ください',
      'negative-request': neg + 'でください',
      permission: te + 'もいい',
      obligation: neg.slice(0, -1) + 'ければならない',
    };
    return M[type] || '';
  }
  if (group === 'godan') {
    const stem = reading.slice(0, -1),
      last = reading.slice(-1);
    let pastEnd = PAST_END[last],
      teEnd = TE_END[last];
    if (reading === 'いく' || reading.endsWith('いく')) {
      pastEnd = 'った';
      teEnd = 'って';
    }
    ...
    const te = stem + teEnd,
      past = stem + pastEnd,
      iStem = stem + I_ROW[last],
      aStem = stem + A_ROW[last],
      eStem = stem + E_ROW[last],
      oStem = stem + O_ROW[last];
    const M = {
      'plain-present': reading,
      'plain-past': past,
      'plain-negative': neg,
      'plain-past-negative': pastNeg,
      'polite-present': iStem + 'ます',
      'polite-past': iStem + 'ました',
      'polite-negative': iStem + 'ません',
      'polite-past-negative': iStem + 'ませんでした',
      'masu-stem': iStem,
      'polite-volitional': iStem + 'ましょう',
      'polite-te': iStem + 'まして',
      'polite-conditional-tara': iStem + 'ましたら',
      'te-form': te,
      potential: eStem + 'る',
      'potential-negative': eStem + 'ない',
      volitional: oStem + 'う',
      'conditional-tara': past + 'ら',
      'negative-conditional-tara': pastNeg + 'ら',
      'conditional-ba': eStem + 'ば',
      'conditional-nara': reading + 'なら',
      conjectural: reading + 'だろう',
      imperative: eStem,
      passive: aStem + 'れる',
      'passive-negative': aStem + 'れない',
      causative: aStem + 'せる',
      'causative-negative': aStem + 'せない',
      'causative-passive': aStem + 'せられる',
      desiderative: iStem + 'たい',
      progressive: te + 'いる',
      'negative-te': neg + 'で',
      prohibition: reading + 'な',
      'request-kudasai': te + 'ください',
  ...
  if (group === 'suru') {
    ...
    const M = {
      ...
      'negative-conditional-tara': 'しなかったら',
      'conditional-ba': 'すれば',
      'conditional-nara': 'するなら',
      conjectural: 'するだろう',
      imperative: 'しろ',
      passive: 'される',
      ...
      'negative-te': 'しないで',
      prohibition: 'するな',
      'request-kudasai': 'してください',
      'negative-request': 'しないでください',
      permission: 'してもいい',
      obligation: 'しなければならない',
    };
    return stem + (M[type] || '');
  }
  if (group === 'kuru') {
    ...
    const M = {
      ...
      'negative-conditional-tara': 'こなかったら',
      'conditional-ba': 'くれば',
      'conditional-nara': 'くるなら',
      conjectural: 'くるだろう',
      imperative: 'こい',
      passive: 'こられる',
      'passive-negative': 'こられない',
      causative: 'こさせる',
      'causative-negative': 'こさせない',
      'causative-passive': 'こさせられる',
      desiderative: 'きたい',
      progressive: 'きている',
      'negative-te': 'こないで',
      prohibition: 'くるな',
      'request-kudasai': 'きてください',
      'negative-request': 'こないでください',
      permission: 'きてもいい',
      obligation: 'こなければならない',
    };
    return stem + (M[type] || '');
  }
  return '';
...
export function conjugateAdjective(adj, type) {
  ...
  if (group === 'i-adjective') {
    ...
    const past = stem + 'かった',
      ...
      pastNeg = stem + 'くなかった';
    const M = {
      'adj-plain-present': reading,
      'adj-plain-past': past,
      'adj-plain-negative': neg,
      'adj-plain-past-negative': pastNeg,
      'adj-polite-present': reading + 'です',
      'adj-polite-past': past + 'です',
      'adj-polite-negative': neg + 'です',
      'adj-polite-past-negative': pastNeg + 'です',
      'adj-te-form': stem + 'くて',
      'adj-negative-te-form': negativeTeConnectiveForm(neg),
      'adj-adverb': stem + 'く',
      'adj-attributive': reading,
      'adj-conditional': stem + 'ければ',
  ...
  if (isAdj) {
    ...
    if (group === 'ichidan') {
      ...
      if (S.startsWith('させら')) {
        change = 'させら';
        suffix = S.slice(4);
      } else if (S.startsWith('させ')) {
        change = 'させ';
        suffix = S.slice(2);
      } else if (S.startsWith('され')) {
        change = 'され';
        suffix = S.slice(2);
      } else if (S.startsWith('でき')) {
        change = 'でき';
        suffix = S.slice(2);
      } else if (S.startsWith('しよ')) {
        change = 'しよ';
        suffix = S.slice(2);
      ...
      if (S.startsWith('させら')) {
        ...
        change = 'させ';
        suffix = S.slice(2);
      } else if (S.startsWith('られ')) {
        change = 'られ';
        suffix = S.slice(2);
      } else if (S.startsWith('よ')) {
        change = 'よ';
        suffix = S.slice(1);
      } else if (S.startsWith('い')) {
        change = 'い';
        suffix = S.slice(1);
      } else if (S.startsWith('れ')) {
        change = 'れ';
        suffix = S.slice(1);
      } else {
        change = '';
...
export function isAdjective(item) {
  return item.group === 'i-adjective' || item.group === 'na-adjective';
}
