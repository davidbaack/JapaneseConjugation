        ...
        <div className="p-5 space-y-4">
          {!supabase ? (
            <div className="space-y-3">
              <div
                role={clientStatus === 'error' ? 'alert' : 'status'}
                aria-live="polite"
                className={`rounded-xl border p-4 text-sm ${
                  clientStatus === 'error'
                    ? 'border-rose-200 bg-rose-50 text-rose-800 dark:border-rose-900 dark:bg-rose-950/20 dark:text-rose-300'
                    : 'border-stone-200 bg-white text-stone-700 dark:border-stone-800 dark:bg-stone-950 dark:text-stone-300'
                }`}
              >
                ...
                <p className="mt-1 text-xs opacity-80">
                  {clientStatus === 'error'
                    ? clientError?.message || 'Check your connection and try again.'
                    : !configured
                      ? 'You can keep practicing locally on this device.'
                      : 'Your local practice remains available while cloud services load.'}
                </p>
              </div>
              {configured && clientStatus === 'error' && (
                <button
                  type="button"
                  onClick={onRetryClient}
