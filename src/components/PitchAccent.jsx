...
export function PitchAccentContour({ morae, pitch }) {
  ...
  return (
    <div className="flex flex-col items-center select-none font-mono">
      <div className="relative" style={{ height: `${height}px`, width: `${morae.length * stepWidth}px` }}>
...
Do not return any extra markdown or chat formatting. Return valid JSON only.`;
      ...
      if (parsed && Array.isArray(parsed.pitch)) {
        const alignedPitch = morae.map((_, i) => (parsed.pitch[i] !== undefined ? parsed.pitch[i] : (parsed.pitch[parsed.pitch.length - 1] || 0)));
        const result = {
          pitch: alignedPitch,
  ...
  const alignedPitch = pitchData ? morae.map((_, i) => (pitchData.pitch[i] !== undefined ? pitchData.pitch[i] : (pitchData.pitch[pitchData.pitch.length - 1] || 0))) : [];
